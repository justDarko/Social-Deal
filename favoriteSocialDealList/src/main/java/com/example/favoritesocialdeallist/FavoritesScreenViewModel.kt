...
import com.example.core.domain.useCase.GetFavoriteSocialDealListUseCase
...
class FavoritesScreenViewModel @Inject constructor(
    private val getFavoriteSocialDealListUseCase: GetFavoriteSocialDealListUseCase,
    ...
    private fun getSocialDealList() {
        viewModelScope.launch {
            getFavoriteSocialDealListUseCase()
                .collect { result ->
                    when (result) {
                        is CustomResult.Success -> {
                            ...
                            _state.value =
                                FavoritesScreenViewState.FavoriteSocialDealsList(result.data)
                        }

                        is CustomResult.Failure -> {
                            Timber.d("Current social deals: ${result.message}")
                            _state.value = FavoritesScreenViewState.Error(result.message)
                        }
                    }
                }
        }
